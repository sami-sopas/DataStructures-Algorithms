...
class List {
  private:
    ...
    /// Sobrecarga iterativa de metodos
...
void List<T, ARRAYSIZE>::mergeSort(const int& leftEdge, const int& rightEdge) {
    ...
    mergeSort(leftEdge, medio);
    mergeSort(medio + 1, rightEdge);

    /// Copiar el conjunto de elementos a ordenar
    ...
    /// Intercalacion
    int i(leftEdge), j(medio + 1), x(i);
    while(i <= medio and j <= rightEdge) {
        while(i <= medio and aux[i] <= aux[j]) {
            data[x++] = aux[i++];
        }
        if(i <= medio) {
...
/// Criterio de paro
    ...
    /// reubiacion de mayores y menores
    while(i < j){
        while(i < j and data[i] <= data[rightEdge]){ /// Pivote
            i++;
        }

        while(i < j and data[j] >= data[rightEdge]){ /// Pivote
            j--;
        }
        if(i != j){
